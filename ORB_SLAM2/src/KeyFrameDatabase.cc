...
void KeyFrameDatabase::add(KeyFrame *pKF) {
	...
	for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend =
			pKF->mBowVec.end(); vit != vend; vit++)
...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF,
	...
	// Discard keyframes connected to the query keyframe
	{
		...
		for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(), vend =
			...
			for (list<KeyFrame*>::iterator lit = lKFs.begin(), lend =
					lKFs.end(); lit != lend; lit++) {
				KeyFrame* pKFi = *lit;
	...
	for (list<pair<float, KeyFrame*> >::iterator it = lScoreAndMatch.begin(),
		...
		for (vector<KeyFrame*>::iterator vit = vpNeighs.begin(), vend =
				vpNeighs.end(); vit != vend; vit++) {
			KeyFrame* pKF2 = *vit;
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F) {
	...
	// Search all keyframes that share a word with current frame
	{
		...
		for (DBoW2::BowVector::const_iterator vit = F->mBowVec.begin(), vend =
				F->mBowVec.end(); vit != vend; vit++) {
			list<KeyFrame*> &lKFs = mvInvertedFile[vit->first];
