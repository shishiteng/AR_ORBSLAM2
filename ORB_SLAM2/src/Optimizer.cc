...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs,
	...
	for (size_t i = 0; i < vpMP.size(); i++) {
		...
		g2o::VertexSBAPointXYZ* vPoint =
				static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(
						pMP->mnId + maxKFid + 1));
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag,
	...
	for (list<KeyFrame*>::iterator lit = lLocalKeyFrames.begin(), lend =
			lLocalKeyFrames.end(); lit != lend; lit++) {
		vector<MapPoint*> vpMPs = (*lit)->GetMapPointMatches();
		for (vector<MapPoint*>::iterator vit = vpMPs.begin(), vend =
				vpMPs.end(); vit != vend; vit++) {
			MapPoint* pMP = *vit;
			if (pMP)
				if (!pMP->isBad())
	...
	for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend =
			lLocalMapPoints.end(); lit != lend; lit++) {
	...
	for (list<KeyFrame*>::iterator lit = lFixedCameras.begin(), lend =
			lFixedCameras.end(); lit != lend; lit++) {
		KeyFrame* pKFi = *lit;
		g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
		vSE3->setEstimate(Converter::toSE3Quat(pKFi->GetPose()));
		vSE3->setId(pKFi->mnId);
...
void Optimizer::OptimizeEssentialGraph(Map* pMap, KeyFrame* pLoopKF,
	...
	for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
		...
		if (it != CorrectedSim3.end()) {
			...
			Eigen::Matrix<double, 3, 3> Rcw = Converter::toMatrix3d(
					pKF->GetRotation());
	...
	for (map<KeyFrame *, set<KeyFrame *> >::const_iterator mit =
			LoopConnections.begin(), mend = LoopConnections.end(); mit != mend;
			mit++) {
		KeyFrame* pKF = mit->first;
		...
		for (set<KeyFrame*>::const_iterator sit = spConnections.begin(), send =
				spConnections.end(); sit != send; sit++) {
	...
	for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
		...
		for (set<KeyFrame*>::const_iterator sit = sLoopEdges.begin(), send =
				sLoopEdges.end(); sit != send; sit++) {
			KeyFrame* pLKF = *sit;
		...
		const vector<KeyFrame*> vpConnectedKFs = pKF->GetCovisiblesByWeight(
				minFeat);
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2,
		vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2,
		const bool bFixScale) {
	g2o::SparseOptimizer optimizer;
