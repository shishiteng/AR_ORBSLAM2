...
bool Initializer::Initialize(const Frame &CurrentFrame,
	...
	for (int it = 0; it < mMaxIterations; it++) {
		...
		for (size_t j = 0; j < 8; j++) {
			int randi = DUtils::Random::RandomInt(0,
...
bool Initializer::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21,
		cv::Mat &K, cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D,
	...
	int nGood1 = CheckRT(R1, t1, mvKeys1, mvKeys2, mvMatches12,
			vbMatchesInliers, K, vP3D1, 4.0 * mSigma2, vbTriangulated1,
			parallax1);
	int nGood2 = CheckRT(R2, t1, mvKeys1, mvKeys2, mvMatches12,
			vbMatchesInliers, K, vP3D2, 4.0 * mSigma2, vbTriangulated2,
			parallax2);
	int nGood3 = CheckRT(R1, t2, mvKeys1, mvKeys2, mvMatches12,
			vbMatchesInliers, K, vP3D3, 4.0 * mSigma2, vbTriangulated3,
			parallax3);
	int nGood4 = CheckRT(R2, t2, mvKeys1, mvKeys2, mvMatches12,
			vbMatchesInliers, K, vP3D4, 4.0 * mSigma2, vbTriangulated4,
			parallax4);
...
bool Initializer::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21,
	...
	for (size_t i = 0; i < 8; i++) {
		...
		int nGood = CheckRT(vR[i], vt[i], mvKeys1, mvKeys2, mvMatches12,
				vbMatchesInliers, K, vP3Di, 4.0 * mSigma2, vbTriangulatedi,
				parallaxi);
...
int Initializer::CheckRT(const cv::Mat &R, const cv::Mat &t,
	...
	for (size_t i = 0, iend = vMatches12.size(); i < iend; i++) {
		...
		vP3D[vMatches12[i].first] = cv::Point3f(p3dC1.at<float>(0),
				p3dC1.at<float>(1), p3dC1.at<float>(2));
		nGood++;
